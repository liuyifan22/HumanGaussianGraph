...
ckpt_path = "/home/zsj/lyf/Wonder3D/ckpts"
...
class MVDiffusionImagePipeline(DiffusionPipeline):
    ...
    def __call__(
        self,
        render_image: Union[List[PIL.Image.Image], torch.FloatTensor],
        ...
        Args:
            render_image (`PIL.Image.Image` or `torch.FloatTensor`): The image that needs to be refined
        ...
        if isinstance(image, list):
            image_pil = image
            render_image_pil = render_image if render_image is not None else image
        elif isinstance(image, torch.Tensor):
            image_pil = [TF.to_pil_image(image[i]) for i in range(image.shape[0])]
            render_image_pil = render_image if render_image is not None else image_pil
        # perhaps the image_embeddings is the cond image!
        
        image_embeddings, image_latents = self._encode_image(image_pil, device, num_images_per_prompt, do_classifier_free_guidance)
        
        dummy, render_image_latents = self._encode_image(render_image_pil, device, num_images_per_prompt, do_classifier_free_guidance)
        # (Pdb) p image_embeddings.shape
        # torch.Size([12, 1, 768])
        # (Pdb) p image_latents.shape
        # torch.Size([12, 4, 32, 32])

        if normal_cond is not None: # it is none
        ...
        """Image Latents and Image Embeddings are always static, only latent is changing"""
        """So we use condition image to generate these two. Use rendered image and normal maps to generate the latent"""
        # but how?
...
import os
def log_validation(dataloader, pipeline, weight_dtype, name, save_dir):
    ...
    for i, batch in tqdm(enumerate(dataloader)):
        ...
        with torch.autocast("cuda"):
            # B*Nv images
            out = pipeline(
                imgs_in, camera_embeddings, generator=None, guidance_scale=guidance_scale, output_type='pt', num_images_per_prompt=1
            ).images
