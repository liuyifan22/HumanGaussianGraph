...
class InterMeshTransformer(nn.Module):
    ...
    def forward(self, mesh_queries):
        ...
        Output:
            mesh_queries: [query_length, 6890, dim]
        """
        neighbor_features = mesh_queries[
            self.batch_idx, self.connectivity_graph, :
        ]  # [5, 6890, 18, channel]
        query_length, vertices, neighborhood_trunc, dim = neighbor_features.shape
        mesh_queries = (
            mesh_queries.reshape(-1, dim).unsqueeze(1).permute(1, 0, 2)
        )  # [5*6890, 1, 128]-> [1, 5*6890, 128]
        neighbor_features = neighbor_features.reshape(
            -1, neighborhood_trunc, dim
        ).permute(
            1, 0, 2
        )  # [5*6890, 18, 128] -> [18, 5*6890, 128]
        connectivity_mask = self.connectivity_mask.reshape(
            -1, neighborhood_trunc
        )  # [5*6890, 18]-> [18, 5*6890]

        mesh_queries = self.attn(
            mesh_queries, neighbor_features, neighbor_features, connectivity_mask
        )
        mesh_queries = mesh_queries.squeeze(0).reshape(
            query_length, vertices, dim
        )  # [5*6890, 128] -> [5, 6890, 128]
        mesh_queries = self.ffn(mesh_queries)
        return mesh_queries
...
class HGG(nn.Module):
    def __init__(
        ...
        connectivity_graph = self.connectivity_graph.clamp(0, 3000000).repeat(
            self.query_length, 1, 1
        )  # [5, 6890, 30]
        connectivity_graph = connectivity_graph[..., : self.neighbourhood_trunc].to(
            device
        )  # [5, 6890, 18]
        connectivity_mask = self.connectivity_mask[
            ..., : self.neighbourhood_trunc
        ].repeat(
            self.query_length, 1, 1
        )  # [5, 6890, 18]
        batch_idx = (
            torch.arange(self.query_length)
            .view(self.query_length, 1, 1)
            .expand(self.query_length, self.vertices, self.neighbourhood_trunc)
            .to(device)
        )
        connectivity_mask = connectivity_mask.reshape(-1, self.neighbourhood_trunc).to(
            device
        )  # [5*6890, 18]-> [18, 5*6890]
        model_path = "./smpl_renderer/mvhuman_tools/visual_smpl/smpl_"
        self.smpl_model = smplx.create(
            gender="neutral", model_type="smpl", model_path=model_path
        )
        ...
        self.mesh_queries = nn.Parameter(
            torch.randn(
                self.vertices, self.query_length, self.gaussian_dim, dtype=torch.float32
            )
        )
        # self.mesh_keys = nn.Parameter(torch.randn(self.faces, 1, self.gaussian_dim, dtype=torch.float32) * 0.1)
        # you can adjust as you like
        self.intra_mesh_transformer_1 = IntraMeshTransformer(
            self.gaussian_dim, num_heads=4
        )
        self.inter_mesh_transformer_1 = InterMeshTransformer(
            self.gaussian_dim,
            num_heads=4,
        ...
        self.intra_mesh_transformer_2 = IntraMeshTransformer(
            self.gaussian_dim, num_heads=4
        )
        self.inter_mesh_transformer_2 = InterMeshTransformer(
            self.gaussian_dim,
            num_heads=4,
            connectivity_graph=connectivity_graph,
            connectivity_mask=connectivity_mask,
        ...
        self.intra_mesh_transformer_3 = IntraMeshTransformer(
            self.gaussian_dim, num_heads=4
        )
        self.inter_mesh_transformer_3 = InterMeshTransformer(
            self.gaussian_dim,
            num_heads=4,
            connectivity_graph=connectivity_graph,
            connectivity_mask=connectivity_mask,
        ...
        self.intra_mesh_transformer_4 = IntraMeshTransformer(
            self.gaussian_dim, num_heads=4
        )
        self.inter_mesh_transformer_4 = InterMeshTransformer(
            self.gaussian_dim,
            num_heads=4,
            connectivity_graph=connectivity_graph,
            connectivity_mask=connectivity_mask,
        ...
        self.intra_mesh_transformer_5 = IntraMeshTransformer(
            self.gaussian_dim, num_heads=4
        )
        self.inter_mesh_transformer_5 = InterMeshTransformer(
            self.gaussian_dim,
            num_heads=4,
            connectivity_graph=connectivity_graph,
            connectivity_mask=connectivity_mask,
        ...
        self.intra_mesh_transformer_6 = IntraMeshTransformer(
            self.gaussian_dim, num_heads=4
        )
        self.inter_mesh_transformer_6 = InterMeshTransformer(
            self.gaussian_dim,
            num_heads=4,
            connectivity_graph=connectivity_graph,
            connectivity_mask=connectivity_mask,
        ...
        self.conv = nn.Conv2d(
            14, 14, kernel_size=1
        )  # NOTE: maybe remove it if train again
    ...
    def prepare_default_rays(self, device, elevation=0):
        ...
        cam_poses = torch.tensor(
            [
                [
                    [-3.2914e-01, -2.9498e-01, 8.9702e-01, 1.9025e00],
                    [2.6506e-02, -9.5247e-01, -3.0349e-01, -1.5521e00],
                    [9.4391e-01, -7.6113e-02, 3.2131e-01, 6.0368e-01],
                    [0.0000e00, 0.0000e00, -2.9802e-08, 1.0000e00],
                ],
                [
                    [-9.4848e-01, 6.7344e-02, -3.0959e-01, -6.8751e-01],
                    [3.2988e-02, -9.5085e-01, -3.0790e-01, -1.5439e00],
                    [-3.1511e-01, -3.0225e-01, 8.9964e-01, 1.8549e00],
                    [0.0000e00, 0.0000e00, 0.0000e00, 1.0000e00],
                ],
                [
                    [3.2914e-01, 2.8916e-01, -8.9892e-01, -1.9389e00],
                    [2.0033e-03, -9.5217e-01, -3.0556e-01, -1.5441e00],
                    [-9.4428e-01, 9.8769e-02, -3.1397e-01, -7.4027e-01],
                    [0.0000e00, 0.0000e00, 0.0000e00, 1.0000e00],
                ],
                [
                    [9.4592e-01, -1.0298e-01, 3.0763e-01, 6.9003e-01],
                    [4.1476e-03, -9.4436e-01, -3.2888e-01, -1.5421e00],
                    [3.2438e-01, 3.1237e-01, -8.9286e-01, -1.9888e00],
                    [0.0000e00, 0.0000e00, 0.0000e00, 1.0000e00],
                ],
            ]
        )
    ...
    def project_points(self, points_3d, view_proj):
        ...
        points_proj = torch.einsum(
            "bvhwk,bvkl->bvhwl", points_3d_h, view_proj
        )  # [bs, views, a, b, 4]
    ...
    def visualize_projected_points(self, projected_points, image_size=(640, 480)):
        ...
        for batch in range(bs):
            for view in range(views):
                ...
                plt.imshow(image)
                plt.scatter(
                    points_2d[:, 0], points_2d[:, 1], c="red", s=10
                )  # Plot points in red
    ...
    def crop(self, img, length=512):
        ...
        img_cropped = img_cropped.view(
            -1, ch, img_cropped.shape[-2], img_cropped.shape[-1]
        )
    ...
    def forward_gaussians(self, images):
        ...
        x = x.reshape(
            B, 4, -1, self.opt.splat_size, self.opt.splat_size
        )  # [B, 4, 14, h, w]
    ...
    def forward(self, video_data, extra_poses=None):
        ...
        for frame_id in frame_ids:
            ...
            features, the_gaussian, raw_gaussian = self.forward_gaussians(
                images
            )  # [B, N, 14]

            ###########################################################################################
            # NOTE: turn on this switch if you want to train LGM
            TRAIN_LGM = False
            ############################################################################################

            if TRAIN_LGM:
                bg_color = torch.ones(
                    3, dtype=torch.float32, device=the_gaussian.device
                )
                the_render_gaussian = the_gaussian.clone().view(
                    features.shape[0], -1, 14
                )  # opacity:3rd
                # 65536
                # opacity > 0.02 : 9415
                # opacity > 0.1 : 8983
                ...
                # [bs, 9000]
                min_dists = (
                    torch.cdist(the_pred_points, smpl_vertices).min(dim=2).values * 10
                )
                min_dist_loss = (
                    dist_loss_k * (min_dists**3)
                ).mean() * features.shape[0]

                loss = loss + min_dist_loss

                # use the other views for rendering and supervision
                the_render_results = self.gs.render(
                    the_render_gaussian,
                    data["cam_view"],
                    data["cam_view_proj"],
                    data["cam_pos"],
                ...
                gt_images = gt_images * gt_masks + bg_color.view(1, 1, 3, 1, 1) * (
                    1 - gt_masks
                )

                loss_mse = F.mse_loss(pred_images, gt_images) + F.mse_loss(
                ...
                if self.opt.lambda_lpips > 0:
                    loss_lpips = self.lpips_loss(
                        ...
                        F.interpolate(
                            gt_images.view(
                                -1, 3, self.opt.output_size, self.opt.output_size
                            )
                            * 2
                            - 1,
                            (256, 256),
                ...
                cat_gaussian = torch.cat(
                    [the_gaussian, features, raw_gaussian], dim=-1
                )  # [B, 4, h, w, 14+128+14]
                the_feature_gaussian = cat_gaussian.clone().view(
                    features.shape[0], -1, 14 + features.shape[-1] + 14
                )  # opacity:3rd
                # 65536
                # opacity > 0.02 : 9415
                # opacity > 0.1 : 8983
                ...
                the_feature_gaussian = torch.stack(
                    selected_feature_gaussian, dim=0
                )  # [1,9000,42]

                smpl_vertices = torch.tensor(
                    data["optimized_vertices"],
                    dtype=the_feature_gaussian.dtype,
                    device=the_feature_gaussian.device,
                )
                # the smpl_vertices and the_render_gaussian are very aligned
                ...
                for b in range(the_feature_gaussian.shape[0]):
                    ...
                    nearest_indices = self.knn_torch_cluster(
                        pred_points, curr_smpl_vertices
                    )  # [num_gaussians]
                    ...
                    unique_vertices, counts = torch.unique_consecutive(
                    ...
                    grouped_tensor = torch.zeros(
                        ...
                        max_gaussians,
                        gaussian_dim,
                        device=sorted_gaussians.device,
                    )
                    ...
                    idx_matrix = (
                        torch.arange(max_count, device=counts.device)
                        .unsqueeze(0)
                        .expand(counts.shape[0], max_count)
                    )
                    ...
                    grouped_tensor[filtered_sorted_values, filtered_local_idx, :] = (
                        filtered_sorted_gaussians
                    )
                ...
                batch_grouped_tensors = torch.stack(
                    batch_grouped_tensors, dim=0
                )  # torch.Size([bs, 6890, 10, 156])
                batch_grouped_masks = torch.stack(
                    batch_grouped_masks, dim=0
                )  # torch.Size([bs, 6890, 10])
                #
        ...
        all_frame_batch_grouped_tensors = torch.stack(
            all_frame_batch_grouped_tensors, dim=0
        )  # torch.Size([5, 1, 6890, 10, 156])
        all_frame_batch_grouped_masks = torch.stack(
            all_frame_batch_grouped_masks, dim=0
        )  # torch.Size([5, 1, 6890, 10])
        all_frame_batch_grouped_tensors = all_frame_batch_grouped_tensors.permute(
            1, 0, 2, 3, 4
        )  # torch.Size([1, 5, 6890, 10, 156])
        all_frame_batch_grouped_masks = all_frame_batch_grouped_masks.permute(
            1, 0, 2, 3
        )  # torch.Size([1, 5, 6890, 10])
        ...
        for bs in range(self.opt.batch_size):
            ...
            features_this_batch = all_frame_batch_grouped_tensors[
                bs
            ]  # [5, 6890, 10, 156]

            # the precomputed masks
            masks_this_batch = all_frame_batch_grouped_masks[bs]  # [5, 6890, 10]
            big_tensor = features_this_batch.permute(1, 0, 2, 3).reshape(
                self.vertices, -1, features_this_batch.shape[-1]
            )  # [6890, 50, 156]
            attn_mask = masks_this_batch.permute(1, 0, 2).reshape(
                self.vertices, -1
            )  # [6890, 50]
            ...
            ############################################################################################
            """# do intra mesh attn"""
            mesh_features = big_tensor[..., 14 : 14 + 128].permute(
                1, 0, 2
            )  # [M, 6890, 128]

            mesh_queries = self.intra_mesh_transformer_1(
                self.mesh_queries.permute(1, 0, 2), mesh_features, attn_mask
            )  # batch second # has residue in it
            mesh_queries = self.inter_mesh_transformer_1(mesh_queries)  # [5, 6890, 128]

            mesh_queries = self.intra_mesh_transformer_2(
                mesh_queries, mesh_features, attn_mask
            )
            mesh_queries = self.inter_mesh_transformer_2(mesh_queries)  # [5, 6890, 128]

            mesh_queries = self.intra_mesh_transformer_3(
                mesh_queries, mesh_features, attn_mask
            )
            mesh_queries = self.inter_mesh_transformer_3(mesh_queries)  # [5, 6890, 128]
            #
            mesh_queries = self.intra_mesh_transformer_4(
                mesh_queries, mesh_features, attn_mask
            )
            mesh_queries = self.inter_mesh_transformer_4(mesh_queries)  # [5, 6890, 128]

            mesh_queries = self.intra_mesh_transformer_5(
                mesh_queries, mesh_features, attn_mask
            )
            mesh_queries = self.inter_mesh_transformer_5(mesh_queries)  # [5, 6890, 128]

            mesh_queries = self.intra_mesh_transformer_6(
                mesh_queries, mesh_features, attn_mask
            )
            ...
            cur_frame_mesh_features = cur_frame_big_tensor[..., 14 : 14 + 128].permute(
                1, 0, 2
            )  # [M, 6890, 128]
            cur_frame_renewed_features = self.gaussian_attn(
                cur_frame_mesh_features, mesh_queries, mesh_queries
            )  # [M, 6890, 128]

            cur_frame_renewed_features = cur_frame_renewed_features.permute(
                1, 0, 2
            )  # [6890, M, 128]
            ...
            cur_frame_original_gaussian = cur_frame_big_tensor[
                ..., 14 + 128 :
            ]  # [6890, M, 14]
            #
            ...
            ones = torch.ones(
                (smpl_vertices_trans.shape[0], 1),
                device=smpl_vertices_trans.device,
                dtype=torch.float32,
            )

            vertices_h = torch.cat(
                [smpl_vertices_trans, ones], dim=1
            )  # shape: (6890, 4)
            ...
            vertices_pre_h = torch.bmm(T_inv, vertices_h.unsqueeze(-1)).squeeze(
                -1
            )  # shape: (6890, 4)
            ...
            for i in range(0, len(frame_ids)):
                ...
                ones = torch.ones(
                    (gaussian_canonical.shape[0], 1),
                    device=gaussian_canonical.device,
                    dtype=torch.float32,
                )
                vertices_h = torch.cat(
                    [gaussian_canonical, ones], dim=1
                )  # shape: (6890, 4)
                vertices_pre_h = torch.bmm(
                    T_for_gaussian, vertices_h.unsqueeze(-1)
                ).squeeze(-1)
                vertices_pre = vertices_pre_h[:, :3] / vertices_pre_h[:, 3].unsqueeze(1)
            ...
            gaussian_for_diff_frames = torch.stack(
                gaussian_for_diff_frames, dim=0
            )  # [8, 13187, 14]
    ...
    def get_transformation_matrices(
        ...
        joints_transforms = output.get(
            "joints_transform", None
        )  # This depends on your SMPLX version.
        ...
        if joints_transforms is None:
            ...
            for j in range(joints.shape[1]):
                ...
                transform = torch.cat(
                    [
                        torch.cat([R, t], dim=-1),
                        torch.tensor([0, 0, 0, 1], device=device).view(1, 1, 4),
                    ],
                    dim=1,
                )
    ...
    def recover_canonical_gaussians(
        self,
        gaussians,
        pose_params,
        translation,
        ...
        trans_dict = self.get_transformation_matrices(
            pose_params, model_path=model_path, device=device
        )
    ...
    def compute_world_to_photo_matrix(
        self, w2c: torch.Tensor, intrinsics: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        K_4x4 = (
            K_4x4.unsqueeze(0).expand(*batch_dims, 4, 4).clone()
        )  # match batch shape
