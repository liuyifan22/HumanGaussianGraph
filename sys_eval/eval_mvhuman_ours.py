...
def process(opt: Options, video_all, time_tag):    
    ...
    for index,frame_id in enumerate(frame_ids):
        ...
        if wonder3d:
            imgs_in = F.interpolate(imgs_in.unsqueeze(0), size=(256, 256), mode='bilinear', align_corners=False)
            ...
            mask = (img_np < 0.98).any(axis=-1)  # True for pixels that are not pure white
            ...
            background = torch.ones((C, 256, 256), dtype=first_img.dtype, device=first_img.device)
            background[:, paste_top:paste_top+h_crop, paste_left:paste_left+w_crop] = cropped
            first_img = background
            
            
            imgs_in = torch.tensor(first_img).unsqueeze(0).repeat(12,1,1,1).to(device)
        
            camera_embeddings = torch.tensor([[0.6610, 0.6610, 0.0000, 1.0000, 0.0000],
                ...
                [0.6610, 0.6666, 5.4768, 0.0000, 1.0000]]).to(device)
            
            
            # smpl_embeddings = torch.zeros(12,10).to(device)
            # camera_embeddings = torch.cat([camera_embeddings, smpl_embeddings], dim=-1)
            out = pipe(
                            imgs_in, camera_embeddings, generator=None, guidance_scale=3.0, output_type='pt', num_images_per_prompt=1
                        ).images
            ...
            for i in range(bsz):
                imageio.imwrite(f"{play_path}/{name}_{i}.png", (mv_image[i] * 255).astype(np.uint8))

            mv_image = np.stack([first_img_ori.permute(1,2,0), mv_image[4], mv_image[3], mv_image[2]], axis=0) # [4, 256, 256, 3], float32
        ...
        with torch.no_grad():
            ...
            for i in range(data["cam_view"].shape[1]): 
                ...
                raster_settings = GaussianRasterizationSettings(
                    ...
                    bg           = torch.tensor([1.0, 1.0, 1.0], dtype=torch.float32, device=gaussians.device),
                    scale_modifier = 1,
                ...
                image, _, _, _ = rasterizer(
                    ...
                    means2D         = torch.zeros_like(gaussians[0, :, 0:3], dtype=torch.float32, device=gaussians.device),
                    shs             = None,
                ...
                # Save result as PNG.
                img_np = (image.squeeze(0).clamp(0, 1).permute(1, 2, 0).detach() * 255).to(torch.uint8).cpu().numpy() # 512, 512, 3
                img_pred = image.squeeze(0).clamp(0, 1).permute(1, 2, 0).detach().cpu().numpy()
                
                img_gt = data["images_output"][0][i].permute(1,2,0).cpu().numpy()
                imageio.imwrite(f"{cur_folder}/{name}_{i}_gt.png", (img_gt * 255).astype(np.uint8))
