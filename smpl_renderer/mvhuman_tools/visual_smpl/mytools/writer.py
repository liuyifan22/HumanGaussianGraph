...
class FileWriter:
    ...
    def vis_smpl(self, render_data, images, cameras, outname, add_back):
        ...
        # get the render results: rgb_img and depth
        render_results, output_depths, face_indices = render.render(render_data, cameras, images, add_back=add_back, ret_depth=True)
        image_vis = merge(render_results, resize=not self.save_origin)
        # depth_vis = merge(output_depths, resize=not self.save_origin)
        output_depths = output_depths[0]
        output_depths = output_depths.reshape(output_depths.shape[0], output_depths.shape[1], 1)
        output_depths = (output_depths - output_depths.min()) / (output_depths.max() - output_depths.min())*255
        # depth_colored= cv2.applyColorMap(output_depths.astype(np.uint8), cv2.COLORMAP_JET)

        cv2.imwrite(outname.replace('.jpg', '_depth.png'), output_depths)
        face_indices_bgr = cv2.cvtColor(face_indices, cv2.COLOR_RGB2BGR)
        cv2.imwrite(outname.replace('.jpg', '_indices.png'), face_indices_bgr)
        return image_vis
