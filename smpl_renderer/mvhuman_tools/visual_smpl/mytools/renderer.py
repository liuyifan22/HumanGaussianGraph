...
# import pyrender
# import numpy as np
# from mvhuman_tools.visual_smpl.mytools.face_index_shader import shader_program
# from OpenGL.GL import glUseProgram

# class CustomRenderer:
#     def __init__(self):
#         self.renderer = pyrender.OffscreenRenderer(viewport_width=640, viewport_height=480)
#         self.shader_program = shader_program  # Use the custom shader program

#     # def render_with_face_indices(self, scene):
#     #     # Use the custom shader program
#     #     glUseProgram(self.shader_program)
#     #     # Render the scene
#     #     rend_rgba, rend_depth = self.renderer.render(scene)
#     #     # Extract face indices from the red channel
#     #     face_indices = (rend_rgba[:, :, 0] * 255).astype(np.int32)
#     #     return face_indices
#     def render_with_face_indices(self, scene):
#         # Use the custom shader program
#         glUseProgram(self.shader_program)
...
#         # Decode face indices from the RGB channels
#         r = rend_rgba[:, :, 0].astype(np.int32)
#         g = rend_rgba[:, :, 1].astype(np.int32)
#         b = rend_rgba[:, :, 2].astype(np.int32)
        
#         # print("Unique values of r:", np.unique(r))
#         # print("Unique values of g:", np.unique(g))
#         # print("Unique values of b:", np.unique(b))
#         face_indices =b
#         # face_indices = (r << 16) + (g << 8) + b
...
class Renderer(object):
    def __init__(self, focal_length=1000, height=512, width=512, faces=None,
        ...
        self.renderer = pyrender.OffscreenRenderer(height, width, point_size=1.0)
        self.faces = faces
    ...
    def add_light(self, scene):
        ...
        light = pyrender.DirectionalLight(color=[1.0, 0.9, 0.6], intensity=1.5) # yellow, dim
    ...
    def render(self, render_data, cameras, images,
        ...
        for nv, img_ in enumerate(images):
            ...
            # self.index_renderer.renderer.viewport_height = img.shape[0]
            # self.index_renderer.renderer.viewport_width = img.shape[1]
            scene = pyrender.Scene(bg_color=self.bg_color,
                                   ambient_light=self.ambient_light)
            scene_for_index = pyrender.Scene(bg_color=[0., 0., 0., 0])
            # scene_for_index = pyrender.Scene(bg_color=self.bg_color,
            #                        ambient_light=self.ambient_light)
            ...
            for iextra, _mesh in enumerate(self.extra_mesh):
                ...
                mesh_copy = mesh_.copy()
                scene.add(mesh_, 'extra{}'.format(iextra))
                scene_for_index.add(mesh_copy, 'extra{}'.format(iextra))
            
            render_data_index = copy.deepcopy(render_data)
            for trackId, data in render_data_index.items():
                # vert, faces and colors all should be np.ndarray
                # color should be in [0, 1]
                faces = data['faces']
                vert = data['vertices'].copy()
                vert = vert @ R.T + T.T
                # use color to store faces ID
                data['colors'] = np.zeros((len(faces), 4))
                ...
                for i in range(len(faces)):
                    ...
                    data['colors'][i] = [r/255., g/255., b/255. , 1.0]  # Normalize to [0, 1] range
                mesh = trimesh.Trimesh(vert, faces, face_colors=data['colors'], process=False)
                mesh.apply_transform(rot)
                # material = pyrender.SimpleMaterial()
                ...
                scene_for_index.add(mesh, data['name'])
            ...
            # mesh_no_light = copy.deepcopy(mesh)
            camera_pose = np.eye(4)
            camera = pyrender.camera.IntrinsicsCamera(fx=K[0, 0], fy=K[1, 1], cx=K[0, 2], cy=K[1, 2])
            scene.add(camera, pose=camera_pose)
            self.add_light(scene)
            # scene_no_light=copy.deepcopy(scene)
            
            # scene_no_light = pyrender.Scene(bg_color=[0.0, 0.0, 0.0, 1.0])
            # scene_no_light.add(mesh_no_light, data['name'])
            # scene_no_light.add(camera, pose=camera_pose)
            scene_for_index.add(camera, pose=camera_pose)
            
            index_flags = pyrender.constants.RenderFlags.FLAT | pyrender.constants.RenderFlags.SKIP_CULL_FACES
            face_indices, face_depth = self.renderer.render(scene_for_index, flags=index_flags)
            
            
            rend_rgba, rend_depth = self.renderer.render(scene, flags=flags)
            # imageio.imwrite('rendered.png', rend_depth)
        ...
        if ret_depth:
            return output_images, output_depths, face_indices
        elif ret_color:
